...
var Puzzle = require('../models/puzzle'); // Import User Model
...
module.exports = function (router) {
    ...
    router.post('/users', function (req, res) {
        ...
        user.name = req.body.firstname + " " + req.body.lastname; // Save name from request to User object
        user.temporarytoken = jwt.sign({ username: user.username, email: user.email }, secret, { expiresIn: '24h' }); // Create a token for activating account

        // Check if request is valid and not empty or null
        if (req.body.username === null || req.body.username === '' || req.body.password === null || req.body.password === '' || req.body.email === null || req.body.email === '' || req.body.name === null || req.body.name === '') {
            res.json({ success: false, message: 'Korisničko ime, ime, lozinka i email su neophodni podaci' });
        } else {
            ...
            user.save(function (err) {
                if (err) {
                    ...
                    if (err.errors !== null) {
                        ...
                        if (err.code == 11000) {
                            if (err.errmsg[61] == "u") {
                                res.json({ success: false, message: 'Korisničko ime je zauzeto' }); // Display error if username already taken
                            } else if (err.errmsg[61] == "e") {
                                res.json({ success: false, message: 'Email je već u upotrebi. Izaberite drugi email' }); // Display error if e-mail already taken
                            }
                        } else {
                    ...
                    res.json({ success: true, message: 'Nalog je registrovan. Molimo sačekajte da Administrator aktivira nalog.' }); // Send success message back to controller/request
                }
            });
        }
    });
    ...
    router.post('/checkusername', function (req, res) {
        ...
        User.findOne({ username: req.body.username }).select('username').exec(function (err, user) {
            if (err) {
                ...
                res.json({ success: false, message: 'Nešto nije kako treba... Greška je na našoj strani. Molim vas izvinite na neprijatnosti, naši inženjeri će popraviti problem što pre!' });
            } else {
                if (user) {
                    res.json({ success: false, message: 'Korisničko ime je već u upotrebi' }); // If user is returned, then username is taken
                } else {
                    res.json({ success: true, message: 'Ispravno Korisničko ime' }); // If user is not returned, then username is not taken
                }
            }
        });
    });
    ...
    router.post('/checkemail', function (req, res) {
        User.findOne({ email: req.body.email }).select('email').exec(function (err, user) {
            if (err) {
                ...
                if (user) {
                    res.json({ success: false, message: 'Uneti e-mail je već u upotrebi' }); // If user is returned, then e-mail is taken
                } else {
                    res.json({ success: true, message: 'Ispravan email!' }); // If user is not returned, then e-mail is not taken
                }
            }
        });
    });
    ...
    router.post('/authenticate', function (req, res) {
        console.log("-------------Api Authenticate: ", req.body);
        ...
        User.findOne({ email: loginUser }).select('email username password').exec(function (err, user) {
            if (err) {
                // Create an e-mail object that contains the error. Set to automatically send it to myself for troubleshooting.
                /*var email = {
                    from: 'MEAN Stack Staff, cruiserweights@zoho.com',
                ...
                if (!user) {
                    res.json({ success: false, message: 'Ne postoji korisnik sa datim korisničkim imenom' }); // Username not found in database
                } else if (user) {
                    // Check if user does exist, then compare password provided by user
                    if (!req.body.password) {
                        res.json({ success: false, message: 'Nije uneta lozinka' }); // Password was not provided
                    } else {
                        var validPassword = user.comparePassword(req.body.password); // Check if password matches password provided by user 
                        if (!validPassword) {
                            res.json({ success: false, message: 'Lozinka nije ispravna' }); // Password does not match password in database
                        } else if (!user.active) {
                            //res.json({ success: false, message: 'Nalog nije aktiviran. Molimo sačekajte da Administrator aktivira nalog', expired: true }); // Account is not activated 
							var token = jwt.sign({ username: user.username, email: user.email }, secret, { expiresIn: '24h' }); // For now we will let everyone login until I make admin activation logic.
                            res.json({ success: true, message: 'Korisnik je aktiviran', token: token });
                        } else {
                            var token = jwt.sign({ username: user.username, email: user.email }, secret, { expiresIn: '24h' }); // Logged in: Give user token
                            res.json({ success: true, message: 'Korisnik je aktiviran', token: token }); // Return token in JSON object to controller
                        }
                    }
                }
            }
        ...
        User.findOne({ temporarytoken: req.params.token }, function (err, user) {
            if (err) {
                ...
                jwt.verify(token, secret, function (err, decoded) {
                    if (err) {
                        res.json({ success: false, message: 'Link za aktivaciju je istekao.' }); // Token is expired
                    } else if (!user) {
                        res.json({ success: false, message: 'Link za aktivaciju je istekao.' }); // Token may be valid but does not match any user in the database
                    } else {
	...
	*/
    ...
    router.put('/resetpassword', function (req, res) {
        User.findOne({ username: req.body.username }).select('username active email resettoken name').exec(function (err, user) {
            if (err) {
                ...
                if (!user) {
                    res.json({ success: false, message: 'Korisničko ime nije pronađeno' }); // Return error if username is not found in database
                } else if (!user.active) {
                    res.json({ success: false, message: 'Nalog nije aktiviran' }); // Return error if account is not yet activated
                } else {
                    ...
                    user.save(function (err) {
                        if (err) {
                            ...
                            res.json({ success: true, message: 'Lozinka je uspešno resetovana!' }); // Return success message
                        }
                    });
                }
            }
    ...
    router.put('/savepassword', function (req, res) {
        User.findOne({ username: req.body.username }).select('username email name password').exec(function (err, user) {
            if (err) {
                // Create an e-mail object that contains the error. Set to automatically send it to myself for troubleshooting.
                /*var email = {
                    from: 'MEAN Stack Staff, cruiserweights@zoho.com',
                ...
                if (req.body.password === null || req.body.password === '') {
                    res.json({ success: false, message: 'Nije uneta lozinka' });
                } else {
                    ...
                    user.save(function (err) {
                        if (err) {
                            ...
                            res.json({ success: true, message: 'Lozinka je promenjena!' }); // Return success message
                        }
                    });
                }
            }
    ...
    router.use(function (req, res, next) {
        ...
        if (token) {
            ...
            jwt.verify(token, secret, function (err, decoded) {
                if (err) {
                    res.json({ success: false, message: 'Nevažeći token' }); // Token has expired or is invalid
                } else {
            ...
            res.json({ success: false, message: 'Token ne postoji' }); // Return error if no token was provided in the request
        }
    });
                    ...
                    if (err) {
                        ...
                        if (!mainUser) {
                            ...
                            if (mainUser.permission === 'admin' || mainUser.permission === 'moderator') {
                                ...
                                if (!users) {
                                    res.json({ success: false, message: 'Nema nijednog korisnika' }); // Return error
                                } else {
                                    res.json({ success: true, users: users, permission: mainUser.permission }); // Return users, along with current user's permission
                                }
                            } else {
                                res.json({ success: false, message: 'Nemate ovlašćenje za pristup ovoj strani' }); // Return access error
                            }
                        }
                    }
                });
            ...
            if (err) {
                ...
                if (!mainUser) {
                    ...
                    if (mainUser.permission !== 'admin') {
                        res.json({ success: false, message: 'Nemate privilegije za ovu operaciju' }); // Return error
                    } else {
    ...
    router.get('/edit/:id', function (req, res) {
        ...
        User.findOne({ username: req.decoded.username }, function (err, mainUser) {
            if (err) {
                ...
                // Check if logged in user was found in database
                if (!mainUser) {
                    res.json({ success: false, message: 'Korisnik nije pronađen' }); // Return error
                } else {
                    ...
                    if (mainUser.permission === 'admin' || mainUser.permission === 'moderator') {
                        ...
                        res.json({ success: false, message: 'Nemate ovlašćenje za ovu operaciju' }); // Return access error
                    }
                }
            }
        });
                ...
                if (!mainUser) {
                    ...
                    if (newUsername) {
                        ...
                        if (mainUser.permission === 'admin' || mainUser.permission === 'moderator') {
                                    ...
                                    if (!user) {
                                        ...
                                        user.save(function (err) {
                                            if (err) {
                                                ...
                                                res.json({ success: true, message: 'Korisničko ime je promenjeno' }); // Return success
                                            }
                                        });
                                    }
                                }
                            ...
                            res.json({ success: false, message: 'Nemate dovoljno ovlašćenja za ovu operaciju' }); // Return error
                        }
                    }
                    ...
                    if (newEmail) {
                        ...
                        if (mainUser.permission === 'admin' || mainUser.permission === 'moderator') {
                                    ...
                                    if (!user) {
                                        ...
                                        user.save(function (err) {
                                            if (err) {
                                                ...
                                                res.json({ success: true, message: 'Email je promenjen' }); // Return success
                                            }
                                        });
                                    }
                                }
                            ...
                            res.json({ success: false, message: 'Nemate ovlašćenja za ovu operaciju' }); // Return error
                        }
                    }
                                                ...
                                                if (mainUser.permission !== 'admin') {
                                                    ...
                                                    user.save(function (err) {
                                                        if (err) {
                                                            ...
                                                            res.json({ success: true, message: 'Ovlašćenja su promenjena' }); // Return success
                                                        }
                                                    });
                                                }
                                            } else {
    ...
    // Route to check the jigsaw word
    router.post('/checkword', function (req, res) {
        var answer = (req.body.answer).toLowerCase();
        Puzzle.findOne({ word: answer }, function (err, data) {
            if (err) {
                res.json({ success: false, message: 'Greška u traženju reči!!!' }); // Return failed message 
            } else {
                if (data == null) {
                    res.json({ success: false, message: "Supervizor nije prihvatio unetu reč" }); // Return failed message  
                } else {
                    res.json({ success: true, message: 'Supervizor prihvata unetu reč' }); // Return success message
                }
            }
        });
    });

    // Route to add the jigsaw word into database
    router.post('/save_puzzle', function (req, res) {
        var word = (req.body.word).toLowerCase();
        Puzzle.create({ word: word }, function (err, docs) {
            if (err != null) {
                res.json({ success: false, message: err }); // Return failed message 
            }
            res.json({ success: true, message: 'Uneta reč je sačuvana!' }); // Return success message
        });
    });
